...
class H264CopyEncoder:
    def __init__(self, info):
        self.__codec_info = info
        self.codec = None
    ...
    def _encode_frame(self, frames, force_keyframe):

        # TODO Check if needed for information offers/ exchanges, else remove 
        if self.codec is None:
            self.codec = av.CodecContext.create('libx264', 'w')
            self.codec.width = self.__codec_info.width
            self.codec.height = self.__codec_info.height
            
            self.codec.pix_fmt = 'yuv420p'
            self.codec.time_base = fractions.Fraction(1, MAX_FRAME_RATE)

            # TODO check if high profile is needed/or supported 
            self.codec.options = {
                'profile': 'high',
                'level': '31',
                'tune': 'zerolatency'
        ...
        yield from self._split_bitstream(b''.join(p.to_bytes() for f in frames))


    def encode(self, frame, force_keyframe=False):
        packages = self._encode_frame(frame, force_keyframe)

        # TODO Check if timebase will be default unless changed 
